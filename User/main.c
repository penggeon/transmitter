...
uint8_t FanMode;    // 风扇模式: 0: 关闭, 1: 打开, 2: 自动

// 标识
uint8_t mainProgramSuccessInit = 0;       // 整体程序是否完成初始化
uint8_t ESP8266GetThresholdSuccess = 0;   // ESP8266是否获取阈值(状态)
uint8_t ESP8266GetThreshold = 0;          // ESP8266获取阈值(动作)
uint8_t ESP8266UpdateData = 0;            // ESP8266上传数据(动作)
uint8_t ESP8266GetFanMode = 0;            // ESP8266获取风扇模式(动作)
// ---- 定义全局变量 ----
